...
def find_best_slice(mask_volume, tumor_labels=[1, 2, 4]):
    ...
    Args:
        mask_volume: 3D numpy array (Z, H, W)
...
def main(args):
    ...
    print("\n--- All Finished! ---")
    print(
        f"Successfully processed and saved data for {processed_count} out of {len(patient_folders)} patients."
    )
    print(f"Bounding box metadata saved to: {metadata_path}")


if __name__ == "__main__":
    arg_parser = argparse.ArgumentParser(
        description="Extract the best slice from a NIfTI file based on tumor area"
    )
